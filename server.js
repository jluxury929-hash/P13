// ===============================================================================
// APEX TITAN v66.2 (HYPER-RESILIENT QUANTUM OMNISCIENT) - ULTIMATE ENGINE
// ===============================================================================
...
process.on('uncaughtException', (err) => {
    ...
    // Shield against protocol mismatches and RPC noise
    ...
    if (msg.includes('401')) {
        console.error("\n\x1b[31m[AUTH ERROR] 401 Unauthorized: Invalid RPC API Key.\x1b[0m");
        return;
    }
...
const GLOBAL_CONFIG = {
    ...
    // 🚦 TRAFFIC CONTROL (v50.0 Hyper-Resilience)
    MAX_CORES: Math.min(os.cpus().length, 12), 
    WORKER_BOOT_DELAY_MS: 10000, // 10s Queue-based stagger
    RPC_COOLDOWN_MS: 12000,
    HEARTBEAT_INTERVAL_MS: 30000,
    RATE_LIMIT_SLEEP_MS: 90000,  // 90s Deep Sleep for Infura safety
    PORT: process.env.PORT || 8080,
    
    // 🐋 OMNISCIENT & QUANTUM SETTINGS
    WHALE_THRESHOLD: parseEther("10.0"), // v50.0 Lower Threshold
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v66.2 | HYPER-RESILIENT OVERLORD      ║
║   MODE: COALESCE SHIELD + QUEUE-BASED DEPLOYMENT      ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);

    const cpuCount = GLOBAL_CONFIG.MAX_CORES;
    console.log(`${TXT.cyan}[SYSTEM] Staggering 12-Core Fleet (10s intervals)...${TXT.reset}`);
    ...
    cluster.on('exit', (worker) => {
        console.log(`${TXT.red}⚠️ Core Offline. Cooldown Restart (30s)...${TXT.reset}`);
...
// --- WORKER PROCESS ---
    ...
    // v50.0: Individual Handshake Jitter
    const initialJitter = 1000 + (cluster.worker.id % 30) * 1500;
    setTimeout(() => {
...
async function initWorker(CHAIN) {
    ...
    try {
        const server = http.createServer((req, res) => {
            if (req.url === '/status') {
                ...
                res.end(JSON.stringify({ status: "ONLINE", role: ROLE, chain: CHAIN.name, mode: "RESILIENT_v66" }));
    ...
    async function safeConnect() {
        ...
        try {
            ...
            // v50.0 FIX: Pre-bind the WebSocket error to catch 429s BEFORE they trigger unhandled rejection
            const wsProvider = IS_LISTENER ? new WebSocketProvider(CHAIN.wss, netObj) : null;
            
            if (wsProvider) {
                wsProvider.on('error', (e) => {
                    const emsg = e.message || "";
                    if (emsg.includes("429") || emsg.includes("coalesce") || emsg.includes("Unexpected server response")) {
                    ...
                    try {
                        ...
                        if (tx && tx.to && tx.value >= GLOBAL_CONFIG.WHALE_THRESHOLD) {
                            ...
                            if (isDEX) {
                                ...
                                console.log(`\n${TAG} ${TXT.magenta}🚨 WHALE INTERCEPT: ${formatEther(tx.value)} ETH!${TXT.reset}`);
                                isProcessing = true;
                ...
                wsProvider.on({ topics: [swapTopic] }, async (log) => {
                    ...
                    try {
                        ...
                        if (maxVal >= GLOBAL_CONFIG.LEVIATHAN_MIN_ETH) {
                            ...
                            isProcessing = true;
                            console.log(`\n${TAG} ${TXT.yellow}🐳 LEVIATHAN LOG: ${formatEther(maxVal)} ETH${TXT.reset}`);
...
async function strike(provider, wallet, fbProvider, iface, pool, CHAIN, target, mode) {
    try {
        ...
        if (simulation && simulation !== "0x") {
            console.log(`\n${TXT.green}${TXT.bold}💎 [${mode}] STRIKE AUTHORIZED!${TXT.reset}`);
