// ===============================================================================
// APEX TITAN FLASH v14.0 - HIGH-FREQUENCY 250 ETH CLUSTER
// ===============================================================================
...
const CONFIG = {
    ...
    // ⚙️ TITAN 250 STRATEGY SETTINGS
    LOAN_AMOUNT: ethers.parseEther("250"), // FIXED 250 ETH "HEAVY" STRIKE
    GAS_LIMIT: 1500000n, // Higher buffer for complex routing
    PRIORITY_BRIBE: 15n, // 15% Tip to be FIRST
    MIN_NET_PROFIT: "0.02" // Minimum net profit floor
};
...
if (cluster.isPrimary) {
    ...
    console.log(`${TXT.bold}${TXT.gold}║   ⚡ TITAN FLASH MASTER | 250 ETH CLUSTER EDITION      ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}╚════════════════════════════════════════════════════════╝${TXT.reset}\n`);
...
async function initWorker() {
    ...
    const server = http.createServer((req, res) => {
        if (req.method === 'GET' && req.url === '/status') {
            ...
            res.end(JSON.stringify({ status: "ONLINE", mode: "TITAN_FLASH_250ETH", target: CONFIG.BENEFICIARY }));
        } else {
            res.writeHead(404);
            res.end();
        }
    ...
    try {
        ...
        console.log(`${TXT.green}✅ TITAN 250 WORKER ACTIVE${TXT.reset} | ${TXT.gold}Treasury: ${ethers.formatEther(balance)} ETH${TXT.reset}`);
        ...
        wsProvider.on("pending", async (txHash) => {
            ...
            if (Math.random() > 0.9995) {
                process.stdout.write(`\n${TXT.magenta}🌊 250 ETH OPPORTUNITY DETECTED: ${txHash.substring(0,10)}...${TXT.reset}\n`);
                await executeTitan250Strike(httpProvider, signer, titanIface, oracleContract, nextNonce, currentEthPrice);
            }
        });
...
async function executeTitan250Strike(provider, signer, iface, oracle, nonce, ethPrice) {
    try {
        console.log(`${TXT.yellow}🔄 CALCULATING TITAN VECTOR (250 ETH)...${TXT.reset}`);
        ...
        // 3. COST ANALYSIS (250 ETH Strategy)
        // Aave V3 Fee: 0.05% of 250 ETH = 0.125 ETH
        ...
        if (netProfit > minProfit) {
            ...
            console.log(`\n${TXT.green}💎 TITAN 250 FLASH CONFIRMED${TXT.reset}`);
